    ...
    const cssModulePath = {
        ...
        childAry: [{
            ...
            childAry: [{
                ...
                /*两边对齐换行*/
                .${moduleName}_flex_between {
                    display: flex;
                    justify-content: space-between;
                    align-items: center;
                }
                /*黄色头*/
                .${moduleName}_yellow_title {
                    height: 40px;
                    width: 100%;
                    font-weight: bold;
...
oaApp.factory("$htNg${firstUpCaseModuleName}01Manager", function($htNgCommManager, $USER_INFO, $TransportService, $${moduleName.toUpperCase()}_CONSTANT, $htNgCommUtil) {
    ...
    factory.fetch${moduleName}ManagerList = function (queryModel, callback, pageCtrl) {
        /**访问服务器获取数据成功执行方法*/
        var getSuccessCall = function (info) {
            pageCtrl.totalItems = info.queryTotalCount;
            callback(info.PageList);
        };
        //var params = "['" + queryModel.userIdList + "','']";
        var params = JSON.stringify([JSON.stringify(queryModel),""]);
        $htNgCommManager.callNewQueryObjectComm("${moduleName}Service", "fetch${firstUpCaseModuleName}ManagerList", params, getSuccessCall, pageCtrl);
    };

    
    factory.fetch${moduleName}ManagerById = function (queryModel,callback) {
        /**访问服务器获取数据成功执行方法*/
        var getSuccessCall = function (info) {
            callback(info);
        };
        //var params = "['" + queryModel.userId + "','']";
        //console.log("fetchKpiManagerList--------------" + params);
        var params = JSON.stringify([JSON.stringify(queryModel),""]);
        $htNgCommManager.callNewObjectComm("${moduleName}Service", "fetch${firstUpCaseModuleName}ManagerById", params, getSuccessCall);
    };/**
    * 获取资产项目的信息(下拉用)
    * @param userInfo
    * @param searchData
    ...
    * @throws Exception
    */
   factory.fetchAssetsGroup = function (callback) {
       /**访问服务器获取数据成功执行方法*/
       var getSuccessCall = function (info) {
           callback(info);
       };
       ...
       //不需翻页伪装成翻页
       $htNgCommManager.callMaskPageFunctionComm("${moduleName}Service", "fetchAssetsGroup", params, getSuccessCall);
   };
...
<div scroll="no" class="h-all" ng-controller="htNg${firstUpCaseModuleName}0100Ctrl">
    <!--<div ng-include="${moduleName}01Model.showCurentPath "></div>-->
    <div ng-style="getNoTabPageHeight()">
        ...
        <!--页面部分-->
        <div ng-include="${moduleName}01Model.showCurentPath">
        </div>
    </div>
</div>`
                        }]
